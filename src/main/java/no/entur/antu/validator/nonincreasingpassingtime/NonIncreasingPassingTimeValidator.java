...
public class NonIncreasingPassingTimeValidator extends AbstractNetexValidator {
  ...
  public void validateServiceJourney(
    ...
    if (!previousPassingTime.isComplete()) {
      validationError.accept(
        previousPassingTime,
        RuleCode.TIMETABLED_PASSING_TIME_INCOMPLETE_TIME
      );
      return;
    }
    if (!previousPassingTime.isConsistent()) {
      validationError.accept(
        previousPassingTime,
        RuleCode.TIMETABLED_PASSING_TIME_INCONSISTENT_TIME
      );
      return;
    }
    ...
    for (int i = 1; i < sortedTimetabledPassingTime.size(); i++) {
      ...
      if (!previousPassingTime.isStopTimesIncreasing(currentPassingTime)) {
        validationError.accept(
          previousPassingTime,
          RuleCode.TIMETABLED_PASSING_TIME_NON_INCREASING_TIME
        );
        return;
      }
  ...
  private String getValidationMessage(RuleCode ruleCode) {
    return switch (ruleCode) {
      ...
      case TIMETABLED_PASSING_TIME_NON_INCREASING_TIME -> "ServiceJourney has non-increasing TimetabledPassingTime";
    };
  }
}
