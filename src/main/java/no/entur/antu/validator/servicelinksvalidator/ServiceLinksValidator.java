...
public class ServiceLinksValidator extends AntuNetexValidator {
  ...
  public void validate(
    ...
    if (
      ...
      List<ServiceLink> serviceLinks = index.getServiceFrames().stream()
      ...
      serviceLinks.stream()
        ...
        .forEach(serviceLinkContext ->
          validateServiceLink(serviceLinkContext,
            error ->
  ...
  private void validateServiceLink(
    ...
    Coordinate geometryStartCoordinate = serviceLinkContext.lineString().getStartPoint().getCoordinate();
    Coordinate geometryEndCoordinate = serviceLinkContext.lineString().getEndPoint().getCoordinate();

    double distanceFromStart = SphericalDistanceLibrary.fastDistance(startCoordinate, geometryStartCoordinate);
    double distanceFromEnd = SphericalDistanceLibrary.fastDistance(endCoordinate, geometryEndCoordinate);

    checkDistanceAndReportError(distanceFromStart, true, serviceLinkContext, reportError);
    checkDistanceAndReportError(distanceFromEnd, false, serviceLinkContext, reportError);
  }
  ...
  private void checkDistanceAndReportError(
    ...
    if (distance > DISTANCE_WARNING) {
      reportError.accept(createServiceLinksError(
        isStart ? ServiceLinksError.RuleCode.DISTANCE_BETWEEN_STOP_POINT_AND_START_OF_LINE_STRING_EXCEEDS_WARNING_LIMIT
      ...
      if (distance > DISTANCE_MAX) {
        reportError.accept(createServiceLinksError(
          isStart ? ServiceLinksError.RuleCode.DISTANCE_BETWEEN_STOP_POINT_AND_START_OF_LINE_STRING_EXCEEDS_MAX_LIMIT
