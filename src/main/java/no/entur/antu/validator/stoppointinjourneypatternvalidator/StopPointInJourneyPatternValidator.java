...
public class StopPointInJourneyPatternValidator extends AbstractNetexValidator {
    ...
    private static final String MISSING_SCHEDULED_STOP_ASSIGNMENT = "MISSING_SCHEDULED_STOP_ASSIGNMENT";
    ...
    private boolean isAssignedToDeadRun(List<DeadRun> deadRuns, String journeyPatternRef) {
        return deadRuns.stream()
                .map(deadRun -> deadRun.getJourneyPatternRef().getValue().getRef())
                .anyMatch(deadRunJourneyPatternRef -> deadRunJourneyPatternRef.equals(journeyPatternRef));
    }

    private boolean isNotAssignedToAnyServiceJourney(List<ServiceJourney> serviceJourneys, String journeyPatternRef) {
        return serviceJourneys.stream()
                .map(serviceJourney -> serviceJourney.getJourneyPatternRef().getValue().getRef())
                .noneMatch(serviceJourneyJourneyPatternRef -> serviceJourneyJourneyPatternRef.equals(journeyPatternRef));
    }

    private boolean validateStopPointInJourneyPattern(NetexEntitiesIndex index,
         ...
         */
        return isAssignedToDeadRun(deadRuns, stopPointInJourneyPatternContext.journeyPattern().getId())
                && isNotAssignedToAnyServiceJourney(serviceJourneys, stopPointInJourneyPatternContext.journeyPattern().getId());
    }
    ...
    public void validate(ValidationReport validationReport, ValidationContext validationContext) {
        ...
        if (validationContext instanceof ValidationContextWithNetexEntitiesIndex validationContextWithNetexEntitiesIndex) {
            ...
            Predicate<StopPointInJourneyPatternContext> hasNoStopPointAssignmentInCommonFile =
                    stopPointInJourneyPatternContext -> commonDataRepository.findQuayIdForScheduledStopPoint(
                            stopPointInJourneyPatternContext.stopPointInJourneyPattern().getScheduledStopPointRef().getValue().getRef(),
                            validationReport.getValidationReportId()) == null;

            Predicate<StopPointInJourneyPatternContext> hasNoStopPointAssignmentInLineFile =
                    stopPointInJourneyPatternContext -> !index.getPassengerStopAssignmentsByStopPointRefIndex().containsKey(
                            stopPointInJourneyPatternContext.stopPointInJourneyPattern().getScheduledStopPointRef().getValue().getRef()
                    );

            stopPointInJourneyPatternContexts.stream()
                    .filter(hasNoStopPointAssignmentInCommonFile.and(hasNoStopPointAssignmentInLineFile))
                    .filter(stopPointInJourneyPatternContext -> !validateStopPointInJourneyPattern(index, stopPointInJourneyPatternContext))
                    .map(StopPointInJourneyPatternContext::stopPointInJourneyPattern)
                    .forEach(stopPointInJourneyPattern -> addValidationReportEntry(
                                    validationReport,
                                    validationContext,
    ...
    private static DataLocation findDataLocation(ValidationContext validationContext,
        ...
        return validationContext.getLocalIds().stream()
                .filter(localId -> localId.getId().equals(stopPointInJourneyPattern.getId()))
                .findFirst()
                .map(idVersion ->
                        new DataLocation(
                                idVersion.getId(),
                ...
                .orElse(new DataLocation(stopPointInJourneyPattern.getId(), fileName, 0, 0));
    }

    @Override
    public Set<String> getRuleDescriptions() {
        return Set.of(
                createRuleDescription(MISSING_SCHEDULED_STOP_ASSIGNMENT, MISSING_SCHEDULED_STOP_ASSIGNMENT_DESCRIPTION)
        );
    }
}
