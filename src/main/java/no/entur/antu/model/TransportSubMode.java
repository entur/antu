...
public record TransportSubMode(String name) {
    ...
    public static Optional<TransportSubMode> from(StopPlace stopPlace) {
        String subModeName = switch (stopPlace.getTransportMode()) {
            ...
            case SNOW_AND_ICE ->
                    stopPlace.getSnowAndIceSubmode() == null ? null : stopPlace.getSnowAndIceSubmode().value();
            default -> throw new AntuException("Unsupported Transport mode in stop place, while getting sub transport mode: " + stopPlace);
        };
        return Optional.ofNullable(subModeName == null ? null : new TransportSubMode(subModeName));
    }
}
