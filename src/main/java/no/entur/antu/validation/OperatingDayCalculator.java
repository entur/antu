...
public class OperatingDayCalculator {
  ...
  public static List<LocalDate> operatingDays(
    ...
    return Stream
      .concat(
        operatingDatesOfOperatingDays(netexEntitiesIndex),
        operatingDatesOfDatesInDayTypeAssignments(netexEntitiesIndex)
      )
      .toList();
  }

  private static Stream<LocalDate> operatingDatesOfOperatingDayRefsInDayTypeAssignments(
    NetexEntitiesIndex netexEntitiesIndex
  ) {
    return netexEntitiesIndex
      ...
      .map(dayTypeAssignment ->
        dayTypeAssignment
          .getOperatingDayRef()
          .getCreated() // TODO: getChanged()?
          .toLocalDate()
      );
  }

  private static Stream<LocalDate> operatingDatesOfDatesInDayTypeAssignments(
    NetexEntitiesIndex netexEntitiesIndex
  ) {
    return netexEntitiesIndex
      .getServiceCalendarFrames()
      ...
      .filter(Objects::nonNull)
      .flatMap(List::stream)
      .map(dayTypeAssignment -> dayTypeAssignment.getDate().toLocalDate());
  }

  private static Stream<LocalDate> operatingDatesOfOperatingDays(
    NetexEntitiesIndex netexEntitiesIndex
  ) {
    return netexEntitiesIndex
      .getServiceCalendarFrames()
      ...
      .filter(Objects::nonNull)
      .flatMap(List::stream)
      .map(operatingDay -> operatingDay.getCalendarDate().toLocalDate());
  }
}
