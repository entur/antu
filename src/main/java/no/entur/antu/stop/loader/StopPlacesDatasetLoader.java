...
@Component
public class StopPlacesDatasetLoader {
    ...
    public StopPlacesDatasetLoader(MardukBlobStoreService mardukBlobStoreService,
                                   @Value("${antu.netex.stop.current.filename:tiamat/CurrentAndFuture_latest.zip}") String currentStopPlacesFile) {
    ...
    public NetexEntitiesIndex loadNetexEntitiesIndex() {
        InputStream currentStopPlaceBlob = mardukBlobStoreService.getBlob(currentStopPlacesFile);
        try (ZipInputStream zipInputStream = new ZipInputStream(currentStopPlaceBlob)) {
