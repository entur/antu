...
public class GooglePubsubConsumer extends DefaultConsumer {
    ...
    protected void doStart() throws Exception {
        super.doStart();
        localLog.info("[Entur] Starting Google PubSub consumer for {}/{}", endpoint.getProjectId(), endpoint.getDestinationName());
    ...
    protected void doStop() throws Exception {
        super.doStop();
        localLog.info("[Entur] Stopping Google PubSub consumer for {}/{}", endpoint.getProjectId(), endpoint.getDestinationName());
        ...
        private void synchronousPull(String subscriptionName) {
            while (isRunAllowed() && !isSuspendingOrSuspended()) {
                try (SubscriberStub subscriber = endpoint.getComponent().getSubscriberStub(endpoint.getServiceAccountKey())) {
                    ...
                    if (e.isRetryable() || statusCode == StatusCode.Code.DEADLINE_EXCEEDED) {
