...
class ValidateNoStopPlacesInImportedFlexDatasetSiteFrameTest {

    public static final String TEST_CODESPACE = "ATB";
    private static final NetexXMLParser NETEX_XML_PARSER = new NetexXMLParser(Set.of());

    private final ValidateNotExist validateStopPlacesNotExist = new ValidateNotExist("SiteFrame/stopPlaces", "stopPlaces not allowed in flexible shared files", "SITE_FRAME_IN_COMMON_FILE_1");
    ...
    void testStopPlacesNotExistsInSiteFrameShouldBeOk() {
        XdmNode document = NETEX_XML_PARSER.parseStringToXdmNode(NETEX_SITE_FRAME_WITH_FLEXIBLE_STOP_PLACES);
        XPathValidationContext xpathValidationContext = new XPathValidationContext(document, NETEX_XML_PARSER, TEST_CODESPACE, null);
        List<XPathValidationReportEntry> xPathValidationReportEntries = validateStopPlacesNotExist.validate(xpathValidationContext);
    ...
    void testStopPlacesExistsInSiteFrameShouldFail() {
        XdmNode document = NETEX_XML_PARSER.parseStringToXdmNode(NETEX_SITE_FRAME_WITH_STOP_PLACES);
        XPathValidationContext xpathValidationContext = new XPathValidationContext(document, NETEX_XML_PARSER, TEST_CODESPACE, null);
        List<XPathValidationReportEntry> xPathValidationReportEntries = validateStopPlacesNotExist.validate(xpathValidationContext);
        Assertions.assertNotNull(xPathValidationReportEntries);
