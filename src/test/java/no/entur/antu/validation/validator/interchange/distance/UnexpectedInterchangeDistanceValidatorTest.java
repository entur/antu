  ...
  void interchangeWithDistanceBetweenStopPointsAboveWarningLimit() {
    ...
    validationReportEntries.forEach(entry ->
      assertThat(
        entry.getName(),
        is(
          UnexpectedInterchangeDistanceError.RuleCode.DISTANCE_BETWEEN_STOP_POINTS_IN_INTERCHANGE_IS_MORE_THAN_WARN.name()
        )
      )
    );
  }
  ...
  void interchangeWithDistanceBetweenStopPointsAboveMaximumLimit() {
    ...
    validationReportEntries.forEach(entry ->
      assertThat(
        entry.getName(),
        is(
          UnexpectedInterchangeDistanceError.RuleCode.DISTANCE_BETWEEN_STOP_POINTS_IN_INTERCHANGE_IS_MORE_THAN_MAX.name()
        )
      )
    );
  }
