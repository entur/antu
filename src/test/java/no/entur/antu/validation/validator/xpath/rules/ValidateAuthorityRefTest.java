...
class ValidateAuthorityRefTest {
    ...
    public void setUp() {
        ...
        when(this.organisationAliasRepository.hasOrganisationWithAlias(EXISTING_ORGANISATION_ALIAS)).thenReturn(true);
        when(this.organisationAliasRepository.hasOrganisationWithAlias(NON_EXISTENT_ORGANISATION_ALIAS)).thenReturn(false);
        this.validator = new ValidateAuthorityRef(organisationAliasRepository);
    }

    @Test
    void testValidAuthorityRef() {
        XPathRuleValidationContext validationContext = validationContextWithAuthorityRef(EXISTING_ORGANISATION_ALIAS);
    ...
    void testInvalidAuthorityRef() {
        XPathRuleValidationContext validationContext = validationContextWithAuthorityRef(NON_EXISTENT_ORGANISATION_ALIAS);
        List<ValidationIssue> validationIssues = validator.validate(validationContext);
        Assertions.assertEquals(1, validationIssues.size());
        Assertions.assertEquals(INVALID_AUTHORITY_REF_RULE, validationIssues.get(0).rule());
    }
    ...
    private XPathRuleValidationContext validationContextWithAuthorityRef(String authorityRef) {
